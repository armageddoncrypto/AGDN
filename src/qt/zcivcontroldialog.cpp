...
std::list<std::string> ZAgdnControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAgdnControlDialog::listMints;

ZAgdnControlDialog::ZAgdnControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAgdnControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAgdnControlDialog::~ZAgdnControlDialog()
{
    delete ui;
}

void ZAgdnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAgdnControlDialog::updateList()
{
...
void ZAgdnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAgdnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAgdn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAgdnControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAgdnControlDialog::GetSelectedMints()
{
...
void ZAgdnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
