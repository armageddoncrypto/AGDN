                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAGDN ought to be enough for anybody." - Bill Gates, 2017
    ui->zAGDNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAGDNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAGDN->setEnabled(false);
        ui->pushButtonMintzAGDN->setToolTip(tr("zAGDN is currently disabled due to maintenance."));

        ui->pushButtonSpendzAGDN->setEnabled(false);
        ui->pushButtonSpendzAGDN->setToolTip(tr("zAGDN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAGDNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAGDN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAGDN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAGDN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAGDN in ") +
...
void PrivacyDialog::on_pushButtonSpendzAGDN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAGDN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAGDN
        sendzAGDN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAGDN
    sendzAGDN();
}

void PrivacyDialog::on_pushButtonZAgdnControl_clicked()
{
    ZAgdnControlDialog* zAgdnControl = new ZAgdnControlDialog(this);
    zAgdnControl->setModel(walletModel);
    zAgdnControl->exec();
}

void PrivacyDialog::setZAgdnControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAgdnSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAGDN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Armageddon Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAGDNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAGDN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AGDN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAGDN</b>";
    ...
    // use mints from zAgdn selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAgdnControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAgdnControlDialog::GetSelectedMints();
    }
    ...
    ZAgdnControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAgdn Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Agdn, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAgdn Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAGDN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAGDN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAGDN "));
    ui->labelzAGDNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzAGDNSyncStatus->setVisible(fShow);
}
